...
module.exports = class Client {
    ...
    request(data) {
        return new Promise(async (resolve, reject) => {
            ...
            this._log(this.levels.debug, 'REQUEST', `Starting new request '${requestId}'.`);
            ...
            setTimeout(() => {
                ...
                this._log(this.levels.info, 'REQUEST', `Request '${requestId}' timed out.`);
            ...
            const requestClient = redis.createClient(this.redisOptions);
            while (!completed) {
                try {
                    const response = await this._getDataFromWorker(data, requestClient);
                    if (completed) {
                        this._log(this.levels.debug, 'REQUEST', `Ignoring response as request '${requestId}' has timed out.`);
                        return;
                    }

                    completed = true;
                    this.runningRequests.finish(requestId);
                    if (response.ok) resolve(response.response);
                    ...
                    if (error.message === 'Could not reach worker') {
                        this._log(this.levels.info, 'REQUEST', `Retrying requst '${requestId}'.`);
                        continue;
                    }
                    ...
                    completed = true;
                    this.runningRequests.finish(requestId);
                    this._log(this.levels.error, 'REQUEST', `Request '${requestId}' failed with unknown error: ${JSON.stringify(error)}.`);
                    reject(error);
                }
            }

            requestClient.end(false);
            this._log(this.levels.debug, 'REQUEST', `Request handling complete for request '${requestId}'.`);
        });
    }
    ...
    async _getDataFromWorker(data, requestClient) {
        return new Promise((resolve, reject) => {
            ...
            this.publisher.spop(this.availabilitySet, (error, requestWorker) => {
                ...
                requestClient.subscribe(keys.responseChannel(requestId), error => {
                    ...
                    const workerChannel = keys.workerChannel(this.queue, requestWorker);
