...
module.exports = class Client {
    ...
    async _getDataFromWorker(data, requestId, requestClient) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                await requestClient.subscribe(keys.responseChannel(requestId));
    ...
    async notifyWorkers() {
        ...
        // All good, a the worker received the request
        if (received > 0)
            return;

        // It's highly unlikely that a worker will be started up so we let the request fail
        this._log(this.levels.notice, 'notify', `There is no active worker. Letting request fail`);
        throw new Error('There is no active worker.');
    }

    _log(level, scope, message) {
        this.logger(level, message, new Date(), 'client', this.id, scope);
    }
